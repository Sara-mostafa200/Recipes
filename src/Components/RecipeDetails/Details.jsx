...
export default function Details() {
  ...
  return (
    <>
      {details.length <= 0 ? (
        <div className="container px-1 flex flex-column justify-between items-center text-center">
          <h1 className="pt-10 text-50">Invalid ID</h1>
          <span className="text-textGray bg-black p-2 text-20">
            Please enter the correct ID to get the recipe
          </span>
        </div>
      ) : (
        <div className="container p-1  text-center">
          {details?.map((item) => (
            <h1 className="text-black ">{item.strMeal}</h1>
          ))}
          {
            ...
            details?.map((item) => (
              <div>
                {" "}
                <div className={`text-black ${style.grid}  `}>
                  <div
                    className={`img flex flex-column  items-center   p-1 ${style.gridChild} `}
                  >
                    <img
                    ...
                    <Link
                      to={item.strYoutube}
                      className={`btn-yellow w-100 mt-1 text-white`}
                      target="_blank"
                    >
                      {" "}
                      Youtube
                  ...
                  <div className={`Instructions ${style.gridChild}    `}>
                    <div className={`bg-backGray shadow rounded-15 p-1`}>
                      <h3 className={`${style.textChange} text-yellow`}>
                        Ingredients
                      </h3>
                      {item.strIngredient1 ? (
                ...
                <div className="Instructions text-start mt-1">
                  <h3>Instructions:</h3>
